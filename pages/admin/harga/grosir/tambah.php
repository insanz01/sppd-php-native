    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">Tambah Grosir</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
      ...
      <div class="col-12">
        <div class="card">
          <div class="card-body">
            ...
            <div class="form-group">
              <button class="btn btn-success btn-block" type="button" role="button" onclick="submitData()">Simpan Data Harga Grosir</button>
            </div>
          </div>
        </div>
      </div>
...
<script>
  const saveData = async (data) => {
    return await axios.post(`<?= $base_url ?>api/add-grosir.api.php`, data, {
      headers: {
  ...
  const submitData = async () => {
    ...
    if(result.status) {
      window.location.href = "<?= $base_url ?>index.php?page=grosir"
    }
  }
