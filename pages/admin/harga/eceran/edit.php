    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">Edit Eceran</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
...
<script>
  const saveData = async (data) => {
    return await axios.post(`<?= $base_url ?>api/edit-eceran.api.php`, data, {
      headers: {
  ...
  const getData = async () => {
    return await axios.get(`<?= $base_url ?>api/get-eceran.api.php?id=<?= $id_edit ?>`).then(res => res.data);
  }
